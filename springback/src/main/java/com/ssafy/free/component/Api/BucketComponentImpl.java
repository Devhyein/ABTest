...
public class BucketComponentImpl implements BucketComponent {
    ...
    @Override
    public int getBucketNumBySessionId(String session_id, int BUCKET_SIZE) {

        // 해당 샘플에서, 버킷 번호를 구하는 방법이 중요하지는 않음
        // 0 ~ 99 의 범위 반환, 동일한 id 인 경우 항상 일관 된 버킷 번호를 반환해 줌
        // return Math.abs(uuid.hashCode()) % 100;

        return Hashing.consistentHash(Hashing.sha256().hashString(session_id, Charsets.UTF_8), BUCKET_SIZE);
    }
}
